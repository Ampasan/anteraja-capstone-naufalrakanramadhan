...
| **System (Laravel 11 Backend / Reverb)** | ✅ Ya | ✅ Ya (Log GPS / Alert Audit) | ✅ Ya (State Marker & SLA Status) | ✅ Ya (Auto-Sort & Escalation) | `order_timestamp`, `reassignment_timestamp` |
...
```mermaid
graph TD
    A[Aplikasi Seluler SATRIA Aktif Bertugas] -->|Sinyal GPS via Laravel Reverb| B[Laravel 11 Backend & Redis Cache Update < 3s]
    B --> C[Laravel Reverb Broadcast Koordinat ke Peta React-Leaflet Admin Hub]
    
    D[Laravel Scheduled SLA Service Hitung Sisa SLA] -->|Loop Per Menit| E[Auto-Sort Panel SLA secara Ascending]
    E --> F{Sisa Waktu SLA Paket?}
    F -- "< 15 Menit / Breached" --> G[Set Risk MERAH - Alert < 5s via Laravel Reverb]
    F -- "15 - 30 Menit" --> H[Set Risk KUNING - Warning]
    F -- "> 30 Menit" --> I[Set Risk HIJAU - Safe State]
    
    J[Kurir Alami Kendala di Lapangan] -->|Kirim Sinyal max 3 Taps| K[Laravel 11 Backend Broadcast Actionable Alert Pop-Up]
    ...
    M --> O[Eloquent ORM Filter Kurir Active & Load < 20 Paket]
    O --> P[Admin Konfirmasi Reassign dalam < 30 Detik]
    P --> Q[Update DB PostgreSQL & Push Rute Baru ke Kurir B via Laravel Reverb]
    Q --> R[Sinkronisasi ETA Otomatis ke Customer Care API]
```

##### Pernyataan Alur Kebutuhan Sistem (EARS Pattern):
* **KETIKA** perangkat seluler kurir SATRIA terhubung ke jaringan internet, sistem **harus** mentransmisikan koordinat GPS (`courier_live_lat`, `courier_live_long`) ke Laravel 11 backend via Laravel Reverb setiap 5 detik.
* **KETIKA** Laravel 11 backend menerima pembaruan telemetri GPS, sistem **harus** memperbarui posisi penanda (*marker*) kurir pada Peta Pemantauan Langsung dalam latensi maksimal 3 detik.
...
| :--- | :--- | :--- | :--- |
| **BR-01** | Telemetri GPS diterima via Laravel Reverb. | Pembaruan penanda kurir pada peta React.js selesai dalam latensi **≤ 3.0 detik**. | Terjadi pemutusan jaringan lokal pada dasbor Admin Hub. |
...
* **F-01: Live Monitoring Map Module**
  * **F-01.1 Telemetry Receiver Service:** Menerima dan memvalidasi payload GPS dari kurir via Laravel Reverb.
...
* **F-04: Quick Incident Reporting Module**
  ...
  * **F-04.2 Actionable Alert Engine:** Layanan Laravel Reverb memicu *pop-up alert* interaktif di dasbor admin.
...
* **Ketika:** 
  ...
  3. Sinyal dikirim via Laravel Reverb ke Laravel 11 backend. Sisa waktu SLA paket `ialx566343618` terhitung tinggal **12 menit** (< 15 menit, memenuhi BR-03).
...
* **Maka:** 
  1. Eloquent ORM memperbarui alokasi resi `ialx566343618` dari Kurir Budi ke Kurir Agus di database PostgreSQL dan Redis Cache dalam waktu **1.2 detik**.
  2. Laravel Reverb mentransmisikan *push notification* rute baru ke aplikasi seluler Kurir Agus dan memperbarui estimasi tiba (*ETA*) di API Customer Care pada pukul 11:45:22 (total durasi pengalihan **22.0 detik**, memenuhi BR-07 < 30 detik).
