...
## 1. Konteks
Fitur **SLA Risk Indicator Panel** (Panel Indikator Risiko SLA) berfungsi untuk menampilkan dan mengurutkan seluruh paket yang belum terkirim secara otomatis berdasarkan sisa waktu SLA terdekat. Fitur ini merujuk langsung pada **PRD-Javascript.md** Bagian 4 (*In-Scope MVP Core P0*) dan Bagian 7 (AC-02) untuk menekan angka *SLA Breached* di bawah 2.5% dan mempercepat identifikasi keterlambatan yang sebelumnya bersifat reaktif.
...
| **System (Express / Node.js)** | ✅ Ya | ✅ Ya (Hitung SLA) | ✅ Ya (Update Status) | ✅ Ya | `Order_ID` |
...
```mermaid
graph TD
    A[Cron Job / Express Service Hitung SLA Remaining Time] --> B[Sistem Ambil Order_Time & Delivery_Time dari DB]
    ...
    E --> H[Express.js Push Alert via Socket.io ke Admin Hub Panel]
    F --> H
    G --> H
...
| **BR-05** | Terjadi perubahan status risiko paket menjadi Merah. | Peringatan suara/visual dikirimkan ke dasbor admin via Socket.io dalam latensi **≤ 5.0 detik**. | Admin mematikan suara notifikasi di pengaturan UI. |
...
* **Maka:** Sistem di backend Express memperbarui `sla_remaining_time` menjadi 10 menit, menetapkan kategori risiko **HIGH_RISK**, dan menampilkan baris resi `akqg208421122` di baris teratas panel SLA dengan warna **Merah Flashing** dalam latensi **2.4 detik** (memenuhi BR-05 ≤ 5 detik).
