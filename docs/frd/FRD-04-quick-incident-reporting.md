...
| **System (Express / Socket.io)** | ✅ Ya | ✅ Ya (Log Alert) | ✅ Ya (Broadcasting) | ✅ Ya | `location_lat_long` |
...
graph TD
    ...
    D --> E[Express Backend Terima Payload & Simpan Log di PostgreSQL]
    E --> F[Socket.io Broadcast Actionable Alert Pop-Up ke Dasbor Admin Hub]
...
* **KETIKA** kurir memilih kategori kendala dan mengklik "Kirim Laporan" pada aplikasi seluler SATRIA, sistem **harus** mentransmisikan sinyal kendala beserta titik lokasi GPS saat itu ke Express backend via Socket.io.
* **KETIKA** backend Express menerima sinyal laporan kendala, sistem **harus** memunculkan notifikasi *pop-up* aksi (*actionable alert*) pada antarmuka Admin Hub dalam waktu < 5 detik tanpa perlu *reload* halaman.
...
* **F-04.2 Real-time Actionable Alert Engine:** Layanan Socket.io di backend yang memicu *pop-up notification* di dasbor admin secara instan.
* **F-04.3 Quick Action Handler:** Modul pemroses tombol aksi cepat pada *pop-up alert* (Pengalihan Rute / Adjust SLA).
* **F-04.4 Incident History Logger:** Pencatat riwayat pelaporan dan penyelesaian kendala di PostgreSQL via Drizzle ORM.
...
* **Maka:** Sinyal dikirim via Socket.io ke Express backend. Pada pukul 11:45:03 (durasi **3.0 detik**, memenuhi BR-02 ≤ 5s), *pop-up alert* warna Merah "Kendalan Motor - Kurir Budi (Resi ialx566343618)" muncul di dasbor Admin Siti dilengkapi tombol aksi "Pengalihan Rute Instan".
